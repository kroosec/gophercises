...
var (
	testInput1 = `<a href="/foo"/><a href="/foo2"/>`
	testInput2 = `<html>
<body>
  <h1>Hello!</h1>
...
func TestBasicScrapLink(t *testing.T) {
	tests := []struct {
		input    string
		expected []Link
	}{
		...
		{testInput4,
			[]Link{{"#", "Login"}, {"/lost", "Lost? Need help?"}, {"https://twitter.com/marcusolsson", "@marcusolsson"}},
		},
	...
	for i, test := range tests {
		links, err := Scrap(strings.NewReader(test.input))
		if err != nil {
			t.Fatalf("Case #%d: error=%+v", i, err)
		}
		if !reflect.DeepEqual(links, test.expected) {
			t.Fatalf("Case #%d: got=%+v, wanted=%+v", i, links, test.expected)
		}
	}
}
